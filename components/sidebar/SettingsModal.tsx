...
export const SettingsModal = ({ isOpen, onClose, onSave }: SettingsModalProps) => {
  ...
  const handleSave = () => {
    ...
    // Dispatch a custom event when settings are saved
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center">
      <div className="w-full max-w-2xl bg-dark-bg border border-[#444] rounded-xl shadow-2xl p-6 relative">
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[#333] text-white rounded transition-all duration-200 hover:bg-[#444] hover:shadow-[0_0_8px_rgba(138,99,210,0.3)] active:transform active:scale-95"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="px-4 py-2 bg-accent-purple text-white rounded transition-all duration-200 hover:bg-purple-hover hover:shadow-[0_0_8px_rgba(138,99,210,0.5)] active:transform active:scale-95"
          >
            Save
          </button>
        </div>
